{{-- Metadata AJAX --}}
<div id="result-meta" data-total="{{ $literatures->total() }}" data-current-page="{{ $literatures->currentPage() }}"
    data-last-page="{{ $literatures->lastPage() }}" hidden>
</div>
...
<div class="rounded-[2rem] border border-slate-200 bg-white p-4 shadow-sm sm:p-6">
    ...
    @if ($literatures->isEmpty())
        <div class="rounded-[1.5rem] border border-dashed border-slate-200 bg-slate-50 p-12 text-center">
            ...
            <p class="mt-2 text-sm text-slate-500">Coba ubah kata kunci, tipe, atau kategori untuk menemukan koleksi
                yang Anda cari.</p>
        </div>
    @else
