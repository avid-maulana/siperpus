...
<style>
    ...
    #skripsiSubmit .sk-spinner {
        display: none;
    }

    #skripsiSubmit.is-loading .sk-label {
        display: none;
    }

    #skripsiSubmit.is-loading .sk-spinner {
        display: inline-flex;
    }
</style>
...
<div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
    ...
    <form id="skripsiSearchForm" action="{{ route('skripsi.index') }}" method="GET" class="w-full">
        ...
        <div class="grid grid-cols-1 gap-3
            ...
            @if (isset($kbks) && $kbks->count())
                <div>
                    <label for="skripsiKbkSelect"
                        class="mb-1.5 block text-xs font-semibold uppercase tracking-wider text-slate-500">
                        KBK
                    </label>

                    <div class="relative">
                        <select id="skripsiKbkSelect" name="kbk" class="h-[52px] w-full appearance-none rounded-xl border border-slate-300 bg-white
                        ...
                        <span class="material-symbols-outlined pointer-events-none absolute right-3 top-1/2
            ...
            <div>
                ...
                <div class="group relative">
                    <div class="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-4">
                        <span class="material-symbols-outlined text-[21px] text-slate-400
                                   transition-colors group-focus-within:text-[#212A37]">
                            search
                        </span>
                    </div>

                    <input id="skripsiSearchInput" type="search" name="search" value="{{ $activeSearch }}"
                        autocomplete="off" spellcheck="false" aria-label="Cari judul skripsi, NIM, atau nama mahasiswa"
                        placeholder="Cari judul, nama mahasiswa, atau NIM..." class="h-[52px] w-full rounded-xl border border-slate-300 bg-white
                               pl-12 pr-28 text-sm text-slate-700 shadow-sm outline-none
                               transition-all duration-200 placeholder:text-slate-400
                               focus:border-[#212A37] focus:ring-4 focus:ring-slate-100">

                    <button id="skripsiInlineClear" type="button" title="Hapus pencarian" class="{{ filled($activeSearch) ? '' : 'hidden' }}
                    ...
                    <button id="skripsiSubmit" type="submit" class="absolute right-1.5 top-1/2 flex h-10 w-[72px] -translate-y-1/2
            ...
            <div>
                <span class="mb-1.5 block text-center text-xs font-semibold uppercase tracking-wider text-slate-500">
                    Reset
                </span>

                <button type="button" id="skripsiResetFilter" title="Reset Filter" class="relative flex h-[52px] w-[54px] items-center justify-center
                    ...
                    @if ($hasActive)
                        <span class="absolute -right-1 -top-1 h-3 w-3 rounded-full
                                       border-2 border-white bg-[#212A37]"></span>
                    @endif
                </button>
            </div>
        ...
        @if ($hasActive)
            <div class="mt-3 flex flex-wrap items-center gap-2">
                ...
                @if (filled($activeSearch))
                    <a href="{{ request()->fullUrlWithQuery(['search' => null]) }}" class="inline-flex items-center gap-1.5 rounded-full bg-slate-100
                ...
                @if (filled($activeKbk))
                    <a href="{{ request()->fullUrlWithQuery(['kbk' => null]) }}" class="inline-flex items-center gap-1.5 rounded-full bg-slate-100
                                       py-1.5 pl-3 pr-2 text-xs font-medium text-slate-600 hover:bg-slate-200">
